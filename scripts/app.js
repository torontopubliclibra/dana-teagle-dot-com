...
let app = {
    ...
    functions: {
        ...
        projectDisplay: (filter) => {
            ...
            app.projects.data.forEach((project) => {
                ...
                project.tags.forEach((tag) => {projectFilters = projectFilters.filter(filter => filter !== tag).concat([tag])})
            })
            ...
            if (projectFilters.length > 3) {
                ...
                projectFilters = projectFilters.sort().map((filter) => {
                    ...
                    if (filter === "All") {filterName = filter}
            ...
            if (app.projects.filter == "All") {projectArray = sortedByYear}

            // if the selected filter is anything else, fill the array with the sorted projects, filtered by the selected filter
            else {projectArray = sortedByYear.filter((project) => project.tags.includes(app.projects.filter))}
            ...
            let formattedProjects = projectArray.map((project) => {
                ...
                if (project.site) {formattedProject.site = `<a href="${project.site}" target="_blank" class="button" title="${project.title} website">Site <i class="fa fa-external-link-square" aria-hidden="true"></i></a>`}
    ...
    events: () => {
        ...
        app.elements.hamburger.click(app.functions.toggleNav);
        ...
        app.elements.scrollTop.click(() => app.functions.scroll("top"))
